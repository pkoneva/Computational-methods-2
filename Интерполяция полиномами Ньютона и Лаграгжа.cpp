...
void e(double* x, double* y) {
	fout << "Абсолютная погрешность вычислений:" << endl;
	for (int i = 0; i < k; i++) fout << "(" << x[i] << ";" << fabs(y[i] - f(x[i]))<< ")" << endl ;
	return;
}
void er(double* x, double* arr, int n) {
	fout << "Относительная погрешность вычислений:" << endl;
	for (int i = 0; i < k; i++) {
		double e = 1;
		for (int j = 0; j < n; j++) {
			e *= (x[i] - arr[j]) / (j+1);
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		...
		arr1[i] = a+step*i;// Равномерные значения сетки
		double d = (2 * i + 1) * PI / (2 * n + 2);
		if (i != 0 && i != n-1) {
			double h = 0.5 * ((b - a) * cos(d) + a + b);// Корни полинома Чебышёва
			arr2[i] = h;
		}
	}
	for (int i = 0; i < k; i++) {
	...
	for (int i = 0; i < n; i++) {
        ...
        fout << "\nИнтерполировение методом Ньютона в равномерной сетке:\n";
	Interpol_Newton(x, arr1, n);
	fout << "\nИнтерполировение методом Ньютона в узлах - корнях полинома Чебышёва:\n";
	Interpol_Newton(x, arr2, n);
	fout << "\nИнтерполировение методом Лагранжа в равномерной сетке:\n";
	Interpol_Lagrange(x, arr1, n);
	fout << "\nИнтерполирование полинома с узлами в корнях полинома Чебышёва:\n";
